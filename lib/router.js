...
Router.route('/sales/:id/l/', {
    ...
    after: function () {
        ...
        ServerSession.set('referralPath', loc);
        Accounts.onLogin(function() {
          Router.go('/dashboard/');
          ServerSession.set('referralPath', '');
        });
        //console.log( ServerSession.get('referralPath'));
    }
});
...
Router.route('/company/:_id', {
    ...
    waitOn: function() { 
      return [subscriptions.subscribe('companies'), subscriptions.subscribe('users'), subscriptions.subscribe('messages'), subscriptions.subscribe('sales'), subscriptions.subscribe('companyTypeSales'), subscriptions.subscribe('threads'), subscriptions.subscribe('leads'), subscriptions.subscribe('quiz'), subscriptions.subscribe('reviews'), Meteor.subscribe('images')];
...
Router.route('/companies/', {
    name: 'companies',
    waitOn: function() { 
      return [subscriptions.subscribe('companies'), subscriptions.subscribe('users'), subscriptions.subscribe('messages'), subscriptions.subscribe('sales'), subscriptions.subscribe('companyTypeSales'), subscriptions.subscribe('threads'), subscriptions.subscribe('leads'), subscriptions.subscribe('reviews'), Meteor.subscribe('images')];
...
Router.onBeforeAction(requireLogin, {except: ['home', 'login', 'companySignup', 'salesSignup', 'terms', 'privacy', 'salesLanding', 'salesLandingWithForm', 'companyLanding', 'contact', 'blogShow', 'blogIndex']});
